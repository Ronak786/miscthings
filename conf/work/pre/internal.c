...
int InitSSL_Ctx(WOLFSSL_CTX* ctx, WOLFSSL_METHOD* method)
{
    ...
    if (InitMutex(&ctx->countMutex) < 0) {
        printf("Mutex error on CTX init");
        return BAD_MUTEX_E;
    }
...
void FreeSSL_Ctx(WOLFSSL_CTX* ctx)
{
    ...
    if (LockMutex(&ctx->countMutex) != 0) {
        printf("Couldn't lock count mutex");
        return;
    }
    ctx->refCount--;
    if (ctx->refCount == 0)
    ...
    if (doFree) {
        printf("CTX ref count down to 0, doing full free");
        ...
        (void)ctx;
        printf("CTX ref count not 0 yet, no free");
    }
}
                ...
                word16 haveStaticECC, int side)
{
    ...
    if (suites == NULL) {
        printf("InitSuites pointer error");
        return;
    }
                  ...
                  const byte* plain, word32 plainSz, RsaKey* key)
{
    ...
    if (sigSz > ENCRYPT_LEN) {
        printf("Signature buffer too big");
        return BUFFER_E;
    }
...
int SetSSL_CTX(WOLFSSL* ssl, WOLFSSL_CTX* ctx)
{
    ...
    if (!newSSL) {
        printf("freeing old ctx to decrement reference count. Switching ctx.");
        wolfSSL_CTX_free(ssl->ctx);
    }

    /* increment CTX reference count */
    if (LockMutex(&ctx->countMutex) != 0) {
        printf("Couldn't lock CTX count mutex");
        return BAD_MUTEX_E;
    }
    ctx->refCount++;
    UnLockMutex(&ctx->countMutex);
    ...
    if (ssl->options.side == WOLFSSL_SERVER_END && !havePSK && !haveAnon)
        if (!ssl->buffers.certificate || !ssl->buffers.certificate->buffer ||
            ...
            printf("Server missing certificate and/or private key");
...
int InitSSL(WOLFSSL* ssl, WOLFSSL_CTX* ctx)
{
    ...
    if (ssl->options.dtls && ssl->options.side == WOLFSSL_SERVER_END) {
        ...
        if (ret != 0) {
            printf("DTLS Cookie Secret error");
            return ret;
        }
    }
...
void FreeHandshakeResources(WOLFSSL* ssl)
{
    ...
    if (ssl->secure_renegotiation && ssl->secure_renegotiation->enabled) {
        printf("Secure Renegotiation needs to retain handshake resources");
        return;
    }
#endif
...
int DtlsPoolInit(WOLFSSL* ssl)
{
    if (ssl->dtls_pool == NULL) {
        ...
        if (pool == NULL) {
            printf("DTLS Buffer Pool Memory error");
            return MEMORY_E;
        }
        else {
            int i;
...
int DtlsPoolSave(WOLFSSL* ssl, const byte *src, int sz)
{
    ...
    if (pool != NULL && pool->used < DTLS_POOL_SZ) {
        ...
        if (pBuf->buffer == NULL) {
            printf("DTLS Buffer Memory error");
            return MEMORY_ERROR;
        }
...
static int Receive(WOLFSSL* ssl, byte* buf, word32 sz)
{
    ...
    if (ssl->ctx->CBIORecv == NULL) {
        printf("Your IO Recv callback is null, please set");
        return -1;
    }
...
{
    printf("Shrinking output buffer\n");
...
int SendBuffered(WOLFSSL* ssl)
{
    if (ssl->ctx->CBIOSend == NULL) {
        printf("Your IO Send callback is null, please set");
    ...
    while (ssl->buffers.outputBuffer.length > 0) {
        ...
        if (sent > (int)ssl->buffers.outputBuffer.length) {
            printf("SendBuffered() out of bounds read");
...
int CheckAvailableSize(WOLFSSL *ssl, int size)
{
    ...
    if (size < 0) {
        printf("CheckAvailableSize() called with negative number");
        return BAD_FUNC_ARG;
    }
                           ...
                           RecordLayerHeader* rh, word16 *size)
{
    ...
    if (rh->pvMajor != ssl->version.major || rh->pvMinor != ssl->version.minor){
        if (ssl->options.side == WOLFSSL_SERVER_END &&
            ...
            printf("Client attempting to connect with different version");
        else if (ssl->options.side == WOLFSSL_CLIENT_END &&
                                 ssl->options.downgrade &&
                                 ssl->options.connectState < FIRST_REPLY_DONE)
            printf("Server attempting to accept with different version");
            ...
            printf("Use version for formatting only in DTLS till ");
        else {
            printf("SSL version error");
...
static int BuildFinished(WOLFSSL* ssl, Hashes* hashes, const byte* sender)
{
    ...
    static int CipherRequires(byte first, byte second, int requirement)
    {
        ...
        default:
            printf("Unsupported cipher suite, CipherRequires ECC");
            return 0;
        }   /* switch */
        }   /* if     */
        ...
        default:
            printf("Unsupported cipher suite, CipherRequires");
            return 0;
        }  /* switch */
...
static int CheckAltNames(DecodedCert* dCert, char* domain)
{
    ...
    printf("Checking AltNames");

    if (dCert)
        altName = dCert->altNames;

    while (altName) {
        printf("    individual AltName check");
                                                                ...
                                                                word32 size)
{
    ...
    *inOutIdx += OPAQUE24_LEN;
    ...
    printf("Loading peer's cert chain");
    ...
    while (listSz) {
        ...
        *inOutIdx += OPAQUE24_LEN;
        ...
        if (ssl->session.chain.count < MAX_CHAIN_DEPTH &&
            ...
            printf("Couldn't store chain cert for session");
        }
#endif
        ...
        *inOutIdx += certSz;
        ...
        totalCerts++;
        printf("    Put another cert into chain");
    }
    ...
    if (count > 0) {
        ...
        ret = ParseCertRelative(dCert, CERT_TYPE, 0, ssl->ctx->cm);
            ...
            tp = GetTrustedPeer(ssl->ctx->cm, dCert->subjectHash,
                                                                 WC_MATCH_NAME);
        #endif /* NO SKID */
        printf("Checking for trusted peer cert");

        if (tp == NULL) {
            /* no trusted peer cert */
            printf("No matching trusted peer cert. Checking CAs");
            FreeDecodedCert(dCert);
        } else if (MatchTrustedPeer(tp, dCert)){
            printf("Found matching trusted peer cert");
            haveTrustPeer = 1;
        } else {
            printf("Trusted peer cert did not match!");
            FreeDecodedCert(dCert);
        }
    }
    ...
    while (count > 1) {
        ...
        if (ret == 0 && dCert->isCA == 0) {
            printf("Chain cert is not a CA, not adding as one");
        }
        else if (ret == 0 && ssl->options.verifyNone) {
            printf("Chain cert not verified by option, not adding as CA");
        }
            ...
            printf("Failed to verify CA from chain");
        }
        else {
            printf("Verified CA from chain and already had it");
        }
        ...
        if (ret == 0) {
            ...
            if (ssl->ctx->cm->ocspEnabled && ssl->ctx->cm->ocspCheckAll) {
                printf("Doing Non Leaf OCSP check");
                ret = CheckCertOCSP(ssl->ctx->cm->ocsp, dCert, NULL);
                doCrlLookup = (ret == OCSP_CERT_UNKNOWN);
                if (ret != 0) {
                    doCrlLookup = 0;
                    printf("\tOCSP Lookup not ok");
                }
            }
#endif /* HAVE_OCSP */
            ...
            if (ret == 0 && doCrlLookup && ssl->ctx->cm->crlEnabled
                ...
                printf("Doing Non Leaf CRL check");
                ret = CheckCertCRL(ssl->ctx->cm->crl, dCert);

                if (ret != 0) {
                    printf("\tCRL check not ok");
                }
            }
#else
            (void)doCrlLookup;
    ...
    if (count) {
        ...
        if (ret == 0) {
            printf("Verified Peer's cert");
            fatal = 0;
        }
        else if (ret == ASN_PARSE_E) {
            printf("Got Peer cert ASN PARSE ERROR, fatal");
            fatal = 1;
        }
        else {
            printf("Failed to verify Peer's cert");
            if (ssl->verifyCallback) {
                printf("\tCallback override available, will continue");
                fatal = 0;
            }
            else {
                printf("\tNo callback override available, fatal");
                fatal = 1;
            }
        }
        ...
        if (fatal == 0 && ssl->secure_renegotiation
            ...
            if (IsEncryptionOn(ssl, 0)) {
                ...
                if (XMEMCMP(dCert->subjectHash,
                            ...
                            SHA_DIGEST_SIZE) != 0) {
                    printf("Peer sent different cert during scr, fatal");
                    fatal = 1;
        ...
        if (dCert->extKeyUsageSet) {
            ...
            if ((ssl->specs.sig_algo == rsa_sa_algo ||
                ...
                printf("KeyUse Digital Sig not set");
        ...
        if (dCert->extExtKeyUsageSet) {
            if (ssl->options.side == WOLFSSL_CLIENT_END) {
                if ((dCert->extExtKeyUsage &
                    ...
                    printf("ExtKeyUse Server Auth not set");
                ...
                if ((dCert->extExtKeyUsage &
                    ...
                    printf("ExtKeyUse Client Auth not set");
                    ret = EXTKEYUSE_AUTH_E;
                }
            }
        }
        ...
        if (!ssl->options.verifyNone && ssl->buffers.domainName.buffer) {
            if (MatchDomainName(dCert->subjectCN, dCert->subjectCNLen,
                ...
                printf("DomainName match on common name failed");
                if (CheckAltNames(dCert,
                                 (char*)ssl->buffers.domainName.buffer) == 0 ) {
                    printf("DomainName match on alt names failed too");
            ...
            case RSAk:
                {
                    ...
                    if (ssl->peerRsaKey == NULL) {
                        ...
                        if (ssl->peerRsaKey == NULL) {
                            printf("PeerRsaKey Memory error");
                            keyRet = MEMORY_E;
                        } else {
            ...
            case ECDSAk:
                {
                    if (ssl->peerEccDsaKey == NULL) {
                        ...
                        if (ssl->peerEccDsaKey == NULL) {
                            printf("PeerEccDsaKey Memory error");
                            return MEMORY_E;
                        }
    ...
    if (ret != 0) {
        if (!ssl->options.verifyNone) {
            ...
            if (ssl->verifyCallback) {
                ...
                if (ok) {
                    printf("Verify callback overriding error!");
                    ret = 0;
                }
                #ifdef SESSION_CERTS
                if (store->discardSessionCerts) {
                    printf("Verify callback requested discard sess certs");
        ...
        if (ssl->verifyCallback) {
            ...
            if (!ok) {
                printf("Verify callback overriding valid certificate!");
                ret = -1;
    ...
    if (ssl->options.verifyNone &&
        ...
        printf("Ignoring CRL problem based on verify setting");
        ret = ssl->error = 0;
    }
                                                      ...
                                                      word32 totalSz, int sniff)
{
    ...
    if (sniff == NO_SNIFF) {
        if (XMEMCMP(input + *inOutIdx, &ssl->hsHashes->verifyHashes,size) != 0){
            printf("Verify finished error on hashes");
    ...
    switch (type) {
        ...
        case hello_request:
            if (ssl->msgsReceived.got_hello_request) {
                printf("Duplicate HelloRequest received");
        ...
        case client_hello:
            if (ssl->msgsReceived.got_client_hello) {
                printf("Duplicate ClientHello received");
                return DUPLICATE_MSG_E;
            }
        ...
        case server_hello:
            if (ssl->msgsReceived.got_server_hello) {
                printf("Duplicate ServerHello received");
                return DUPLICATE_MSG_E;
            }
        ...
        case hello_verify_request:
            if (ssl->msgsReceived.got_hello_verify_request) {
                printf("Duplicate HelloVerifyRequest received");
                return DUPLICATE_MSG_E;
            }
        ...
        case session_ticket:
            if (ssl->msgsReceived.got_session_ticket) {
                printf("Duplicate SessionTicket received");
                return DUPLICATE_MSG_E;
            }
        ...
        case certificate:
            if (ssl->msgsReceived.got_certificate) {
                printf("Duplicate Certificate received");
                return DUPLICATE_MSG_E;
            }
            ...
            if (ssl->options.side == WOLFSSL_CLIENT_END) {
                if ( ssl->msgsReceived.got_server_hello == 0) {
                    printf("No ServerHello before Cert");
            ...
            if (ssl->options.side == WOLFSSL_SERVER_END) {
                if ( ssl->msgsReceived.got_client_hello == 0) {
                    printf("No ClientHello before Cert");
                    return OUT_OF_ORDER_E;
                }
            }
#endif
        ...
        case certificate_status:
            if (ssl->msgsReceived.got_certificate_status) {
                printf("Duplicate CertificateSatatus received");
                return DUPLICATE_MSG_E;
            }
            ssl->msgsReceived.got_certificate_status = 1;

            if (ssl->msgsReceived.got_certificate == 0) {
                printf("No Certificate before CertificateStatus");
                return OUT_OF_ORDER_E;
            }
            if (ssl->msgsReceived.got_server_key_exchange != 0) {
                printf("CertificateStatus after ServerKeyExchange");
                return OUT_OF_ORDER_E;
            }
        ...
        case server_key_exchange:
            if (ssl->msgsReceived.got_server_key_exchange) {
                printf("Duplicate ServerKeyExchange received");
                return DUPLICATE_MSG_E;
            }
            ssl->msgsReceived.got_server_key_exchange = 1;

            if (ssl->msgsReceived.got_server_hello == 0) {
                printf("No ServerHello before ServerKeyExchange");
                return OUT_OF_ORDER_E;
            }
            if (ssl->msgsReceived.got_certificate_status == 0) {
                ...
                if (ssl->status_request) {
                    ...
                    printf("No CertificateStatus before ServerKeyExchange");
        ...
        case certificate_request:
            if (ssl->msgsReceived.got_certificate_request) {
                printf("Duplicate CertificateRequest received");
                return DUPLICATE_MSG_E;
            }
        ...
        case server_hello_done:
            if (ssl->msgsReceived.got_server_hello_done) {
                printf("Duplicate ServerHelloDone received");
                return DUPLICATE_MSG_E;
            }
            ...
            if (ssl->msgsReceived.got_certificate == 0) {
                if (ssl->specs.kea == psk_kea ||
                    ...
                    printf("No Cert required");
                } else {
                    printf("No Certificate before ServerHelloDone");
                    return OUT_OF_ORDER_E;
                }
            }
            if (ssl->msgsReceived.got_server_key_exchange == 0) {
                ...
                if (ssl->specs.static_ecdh == 1 ||
                    ...
                    pskNoServerHint) {
                    printf("No KeyExchange required");
                } else {
                    printf("No ServerKeyExchange before ServerDone");
                    return OUT_OF_ORDER_E;
                }
            }
            break;
        ...
        case certificate_verify:
            if (ssl->msgsReceived.got_certificate_verify) {
                printf("Duplicate CertificateVerify received");
                return DUPLICATE_MSG_E;
            }
            ssl->msgsReceived.got_certificate_verify = 1;

            if ( ssl->msgsReceived.got_certificate == 0) {
                printf("No Cert before CertVerify");
                return OUT_OF_ORDER_E;
            }
            break;
#endif
        ...
        case client_key_exchange:
            if (ssl->msgsReceived.got_client_key_exchange) {
                printf("Duplicate ClientKeyExchange received");
                return DUPLICATE_MSG_E;
            }
            ssl->msgsReceived.got_client_key_exchange = 1;

            if (ssl->msgsReceived.got_client_hello == 0) {
                printf("No ClientHello before ClientKeyExchange");
                return OUT_OF_ORDER_E;
            }
            break;
#endif
        ...
        case finished:
            if (ssl->msgsReceived.got_finished) {
                printf("Duplicate Finished received");
                return DUPLICATE_MSG_E;
            }
            ssl->msgsReceived.got_finished = 1;

            if (ssl->msgsReceived.got_change_cipher == 0) {
                printf("Finished received before ChangeCipher");
        ...
        case change_cipher_hs:
            if (ssl->msgsReceived.got_change_cipher) {
                printf("Duplicate ChangeCipher received");
                return DUPLICATE_MSG_E;
            }
            ...
            if (ssl->options.side == WOLFSSL_CLIENT_END) {
                if (!ssl->options.resuming &&
                    ...
                    printf("No ServerHelloDone before ChangeCipher");
                    return OUT_OF_ORDER_E;
                }
            }
#endif
            ...
            if (ssl->options.side == WOLFSSL_SERVER_END) {
                if (!ssl->options.resuming &&
                    ...
                    printf("No ClientKeyExchange before ChangeCipher");
                    return OUT_OF_ORDER_E;
                }
            }
#endif
                          ...
                          byte type, word32 size, word32 totalSz)
{
    ...
    if ( (ret = SanityCheckMsgReceived(ssl, type)) != 0) {
        printf("Sanity Check on handshake message type received failed");
        return ret;
    }
    ...
    if (ssl->options.handShakeState == HANDSHAKE_DONE && type != hello_request){
        printf("HandShake message after handshake complete");
    ...
    if (ssl->options.side == WOLFSSL_CLIENT_END && ssl->options.dtls == 0 &&
        ...
        printf("First server message not server hello");
        SendAlert(ssl, alert_fatal, unexpected_message);
        return OUT_OF_ORDER_E;
    }
    ...
    if (ssl->options.side == WOLFSSL_CLIENT_END && ssl->options.dtls &&
        ...
        printf("Server hello done received before server hello in DTLS");
        SendAlert(ssl, alert_fatal, unexpected_message);
        return OUT_OF_ORDER_E;
    }
    ...
    if (ssl->options.side == WOLFSSL_SERVER_END &&
        ...
        printf("First client message not client hello");
        SendAlert(ssl, alert_fatal, unexpected_message);
        return OUT_OF_ORDER_E;
    }
    ...
    case hello_request:
        printf("processing hello request");
        ret = DoHelloRequest(ssl, input, inOutIdx, size, totalSz);
        break;

#ifndef NO_WOLFSSL_CLIENT
    case hello_verify_request:
        printf("processing hello verify request");
        ret = DoHelloVerifyRequest(ssl, input,inOutIdx, size);
        break;

    case server_hello:
        printf("processing server hello");
        ret = DoServerHello(ssl, input, inOutIdx, size);
        break;

#ifndef NO_CERTS
    case certificate_request:
        printf("processing certificate request");
        ret = DoCertificateRequest(ssl, input, inOutIdx, size);
        break;
#endif

    case server_key_exchange:
        printf("processing server key exchange");
        ret = DoServerKeyExchange(ssl, input, inOutIdx, size);
        break;

#ifdef HAVE_SESSION_TICKET
    case session_ticket:
        printf("processing session ticket");
...
#ifndef NO_CERTS
    case certificate:
        printf("processing certificate");
        ret = DoCertificate(ssl, input, inOutIdx, size);
        break;

    case certificate_status:
        printf("processing certificate status");
        ret = DoCertificateStatus(ssl, input, inOutIdx, size);
        break;
#endif

    case server_hello_done:
        printf("processing server hello done");
        #ifdef WOLFSSL_CALLBACKS
            if (ssl->hsInfoOn)
        ...
        if (ssl->options.resuming) {
            printf("Not resuming as thought");
...
#ifndef NO_WOLFSSL_SERVER
    case client_hello:
        printf("processing client hello");
        ret = DoClientHello(ssl, input, inOutIdx, size);
        break;

    case client_key_exchange:
        printf("processing client key exchange");
        ret = DoClientKeyExchange(ssl, input, inOutIdx, size);
        break;

#if !defined(NO_RSA) || defined(HAVE_ECC)
    case certificate_verify:
        printf("processing certificate verify");
    ...
    default:
        printf("Unknown handshake message type");
                          ...
                          word32 totalSz)
{
    ...
    if (ssl->arrays->pendingMsgSz == 0) {
        ...
        if (size > MAX_HANDSHAKE_SZ) {
            printf("Handshake message too large");
...
static INLINE int Encrypt(WOLFSSL* ssl, byte* out, const byte* input, word16 sz)
{
    ...
    if (ssl->encrypt.setup == 0) {
        printf("Encrypt ciphers not setup");
    ...
    switch (ssl->specs.bulk_cipher_algorithm) {
            ...
            default:
                printf("wolfSSL Encrypt programming error");
                ret = ENCRYPT_ERROR;
    }
                           ...
                           word16 sz)
{
    ...
    if (ssl->decrypt.setup == 0) {
        printf("Decrypt ciphers not setup");
    ...
    switch (ssl->specs.bulk_cipher_algorithm) {
            ...
            default:
                printf("wolfSSL Decrypt programming error");
                ret = DECRYPT_ERROR;
    }
...
static int SanityCheckCipherText(WOLFSSL* ssl, word32 encryptSz)
{
    ...
    if (ssl->specs.cipher_type == block) {
        if (encryptSz % ssl->specs.block_size) {
            printf("Block ciphertext not block size");
    ...
    if (encryptSz < minLength) {
        printf("Ciphertext not minimum size");
        return SANITY_CIPHER_E;
    }
                           ...
                           int pLen, int content)
{
    ...
    if ( (t + padLen + 1) > pLen) {
        printf("Plain Len not long enough for pad/mac");
    ...
    if (ConstantCompare(verify, input + (pLen - padLen - 1 - t), t) != 0) {
        printf("Verify MAC compare failed");
        return VERIFY_MAC_ERROR;
    }
...
int DoApplicationData(WOLFSSL* ssl, byte* input, word32* inOutIdx)
{
    ...
    if (ssl->options.handShakeDone == 0) {
        printf("Received App data before a handshake completed");
        SendAlert(ssl, alert_fatal, unexpected_message);
        return OUT_OF_ORDER_E;
    }
    ...
    if (dataSz < 0) {
        printf("App data buffer error, malicious input?");
        return BUFFER_ERROR;
    }
    ...
    for (;;) {
        ...
        case doProcessInit:
            ...
            if ( ssl->options.side == WOLFSSL_SERVER_END &&
                ...
                if (ssl->buffers.inputBuffer.buffer[
                    ...
                    printf("Not a valid old client hello");
                    return PARSE_ERROR;
                }
                ...
                if (ssl->buffers.inputBuffer.buffer[
                    ...
                    printf("Not a valid version in old client hello");
                    return PARSE_ERROR;
                }
        ...
        case getRecordLayerHeader:
            ...
            if (ssl->options.dtls && ret == SEQUENCE_ERROR) {
                printf("Silently dropping out of order DTLS message");
                ssl->options.processReply = doProcessInit;
        ...
        case runProcessingOneMessage:
            ...
            printf("received record layer msg");
            ...
            switch (ssl->curRL.type) {
                ...
                case change_cipher_spec:
                    ...
                    if (ssl->options.dtls &&
                        ...
                        printf("Duplicate ChangeCipher msg");
                        ret = DtlsPoolSend(ssl);
                        if (ret != 0)
                            return ret;

                        if (ssl->curSize != 1) {
                            printf("Malicious or corrupted"
                    ...
                    if (ssl->options.side == WOLFSSL_CLIENT_END &&
                        ...
                        printf("Expected session ticket missing");
                    ...
                    if (ssl->curSize != 1) {
                        printf("Malicious or corrupted ChangeCipher msg");
                        return LENGTH_ERROR;
                    }
                    #ifndef NO_CERTS
                        if (ssl->options.side == WOLFSSL_SERVER_END &&
                            ...
                            if (!ssl->options.havePeerVerify) {
                                printf("client didn't send cert verify");
            ...
            /* more messages per record */
                ...
                printf("More messages in record");
                #ifdef WOLFSSL_DTLS
                ...
                if (IsEncryptionOn(ssl, 0)) {
                    printf("Bundled encrypted messages, remove middle pad");
                    ssl->buffers.inputBuffer.idx -= ssl->keys.padSz;
                }
            ...
            /* more records */
                ...
                printf("More records in input");
                ssl->options.processReply = doProcessInit;
                continue;
            }

        default:
            printf("Bad process input state, programming error");
            return INPUT_CASE_ERROR;
        }
    }
}
...
#endif
    if (sz > (word32)outSz) {
        printf("Oops, want to write past output buffer size");
        return BUFFER_E;
    }
...
int SendData(WOLFSSL* ssl, const void* data, int sz)
{
    ...
    if (ssl->options.handShakeState != HANDSHAKE_DONE) {
        int err;
        printf("handshake not complete, trying to finish");
    ...
    if (ssl->buffers.outputBuffer.length > 0) {
        printf("output buffer was full, trying to send again");
        if ( (ssl->error = SendBuffered(ssl)) < 0) {
            ...
            printf("sent write buffered data");

            if (sent > sz) {
                printf("error: write() after WANT_WRITE with short size");
    ...
    for (;;) {
        ...
        if (ssl->options.partialWrite == 1) {
            printf("Paritial Write on, only sending one record");
            break;
        }
    }
...
int ReceiveData(WOLFSSL* ssl, byte* output, int sz, int peek)
{
    ...
    if (ssl->error != 0 && ssl->error != WANT_WRITE) {
        printf("User calling wolfSSL_read in error state, not allowed");
        return ssl->error;
    }
...
startScr:
    if (ssl->secure_renegotiation && ssl->secure_renegotiation->startScr) {
        ...
        printf("Need to start scr, server requested");
    ...
    while (ssl->buffers.clearOutputBuffer.length == 0) {
        if ( (ssl->error = ProcessReply(ssl)) < 0) {
            ...
            if (ssl->error == ZERO_RETURN) {
                printf("Zero return, no more data coming");
                return 0;         /* no more data coming */
            }
            if (ssl->error == SOCKET_ERROR_E) {
                if (ssl->options.connReset || ssl->options.isClosed) {
                    printf("Peer reset or closed, connection done");
...
int SetCipherList(Suites* suites, const char* list)
{
    ...
    if (suites == NULL || list == NULL) {
        printf("SetCipherList parameter error");
        return 0;
    }
...
/* client only parts */
    ...
    int SendClientHello(WOLFSSL* ssl)
    {
        ...
        if (ssl->suites == NULL) {
            printf("Bad suites pointer in SendClientHello");
            return SUITES_ERROR;
        }
                             ...
                             word32 helloSz)
    {
        ...
        if (pv.minor > ssl->version.minor) {
            printf("Server using higher version, fatal error");
            return VERSION_ERROR;
        }
        else if (pv.minor < ssl->version.minor) {
            printf("server using lower version");

            if (!ssl->options.downgrade) {
                printf("    no downgrade allowed, fatal error");
                return VERSION_ERROR;
            }
            if (pv.minor < ssl->options.minDowngrade) {
                printf("    version below minimum allowed, fatal error");
                return VERSION_ERROR;
            }
                ...
                if (ssl->secure_renegotiation &&
                    ...
                    printf("Server changed version during scr");
                    return VERSION_ERROR;
                }
            #endif
            ...
            if (pv.minor == SSLv3_MINOR) {
                ...
                /* turn off tls 1.1+ */
                printf("    downgrading to TLSv1");
                ssl->options.tls1_1 = 0;
                ssl->version.minor  = TLSv1_MINOR;
            }
            else if (pv.minor == TLSv1_1_MINOR) {
                printf("    downgrading to TLSv1.1");
        ...
        if (ssl->arrays->sessionIDSz > ID_LEN) {
            printf("Invalid session ID size");
        ...
        if (ssl->secure_renegotiation && ssl->secure_renegotiation->enabled &&
            ...
            if (ssl->options.cipherSuite0 != cs0 ||
                ...
                printf("Server changed cipher suite during scr");
        ...
        if (compression != ZLIB_COMPRESSION && ssl->options.usingCompression) {
            printf("Server refused compression, turning off");
        ...
        if (ssl->options.resuming) {
            if (DSH_CheckSessionId(ssl)) {
                if (SetCipherSpecs(ssl) == 0) {
                    ...
                    printf("Unsupported cipher suite, DoServerHello");
                    return UNSUPPORTED_SUITE;
                }
            }
            else {
                printf("Server denied resumption attempt");
        ...
        case diffie_hellman_kea:
        {
            ...
            *inOutIdx += OPAQUE16_LEN;
            ...
            if (length < ssl->options.minDhKeySz) {
                printf("Server using a DH key that is too small");
            ...
            *inOutIdx += 1;   /* curve type, eat leading 0 */
            ...
            if (ssl->peerEccKey == NULL) {
                ...
                ssl->peerEccKey = (ecc_key*)XMALLOC(sizeof(ecc_key),
                                             ssl->ctx->heap, DYNAMIC_TYPE_ECC);
                if (ssl->peerEccKey == NULL) {
                    printf("PeerEccKey Memory error");
                    return MEMORY_E;
                }
        ...
        byte* bufOut, word16* outSz)
{
    ...
    switch (key->name) {
        ...
        case WOLFSSL_NTRU_EESS743:
            break;
        default:
            printf("Unknown QSH encryption key!");
            return -1;
    }
        ...
        byte* bufOut, word16* outSz)
{
    ...
    switch (key->name) {
        ...
        case WOLFSSL_NTRU_EESS743:
            break;
        default:
            printf("Unknown QSH decryption key!");
            return -1;
    }
                                                       ...
                                                       byte* out, word32* szOut)
{
    ...
    printf("Encrypting QSH key material");
                                                       ...
                                                       byte* out, word16* szOut)
{
    ...
    printf("Decrypting QSH key material");
...
#endif
        default:
            printf("Unknown QSH encryption scheme size!");
            return 0;
    }
...
static int QSH_GenerateSerCliSecret(WOLFSSL* ssl, byte isServer)
{
    ...
    printf("Generating QSH secret key material");
...
static word32 QSH_KeyExchangeWrite(WOLFSSL* ssl, byte isServer)
{
            ...
            case psk_kea:
                {
                    ...
                    if (ssl->options.client_psk_cb == NULL) {
                        printf("No client PSK callback set");
    ...
    int SendCertificateVerify(WOLFSSL* ssl)
    {
        ...
        {
    #ifdef HAVE_ECC
            printf("Trying ECC client cert, RSA didn't work");

            if (ssl->buffers.key == NULL) {
                printf("ECC Key missing");
                return NO_PRIVATE_KEY;
            }
            ...
            ret = wc_EccPrivateKeyDecode(ssl->buffers.key->buffer, &idx, &eccKey,
                                      ssl->buffers.key->length);
            if (ret == 0) {
                printf("Using ECC client cert");
                usingEcc = 1;
                sigOutSz = MAX_ENCODED_SIG_SZ;
            }
            else {
                printf("Bad client cert type");
            }
    #endif
        }
        if (ret == 0) {
                               ...
                               const byte* input, word32* inOutIdx, word32 size)
{
    ...
    if (ssl->expect_session_ticket == 0) {
        printf("Unexpected session ticket");
        return SESSION_TICKET_EXPECT_E;
    }
    ...
    *inOutIdx += OPAQUE16_LEN;
    ...
    int SendServerKeyExchange(WOLFSSL* ssl)
    {
        ...
        case ecdhe_psk_kea:
        {
            ...
            if (ssl->eccTempKey == NULL) {
                ...
                ssl->eccTempKey = (ecc_key*)XMALLOC(sizeof(ecc_key),
                                             ssl->ctx->heap, DYNAMIC_TYPE_ECC);
                if (ssl->eccTempKey == NULL) {
                    printf("EccTempKey Memory error");
                    return MEMORY_E;
                }
        ...
        case ecc_diffie_hellman_kea:
        {
            ...
            if (ssl->specs.static_ecdh) {
                printf("Using Static ECDH, not sending ServerKeyExchange");
                return 0;
            }
    ...
    static int VerifyServerSuite(WOLFSSL* ssl, word16 idx)
    {
        ...
        if (CipherRequires(first, second, REQUIRES_NTRU)) {
            printf("Requires NTRU");
            if (ssl->options.haveNTRU == 0) {
                printf("Don't have NTRU");
                return 0;
            }
        }

        if (CipherRequires(first, second, REQUIRES_RSA_SIG)) {
            printf("Requires RSA Signature");
            if (ssl->options.side == WOLFSSL_SERVER_END &&
                                           ssl->options.haveECDSAsig == 1) {
                printf("Don't have RSA Signature");
                return 0;
            }
        }
        ...
        if (!TLSX_ValidateEllipticCurves(ssl, first, second)) {
            printf("Don't have matching curves");
                return 0;
        }
#endif
        ...
        if (first == QSH_BYTE && second == TLS_QSH) {
            if (TLSX_SupportExtensions(ssl)) {
                ...
                printf("Version of SSL connection does not support TLS_QSH");
            }
            return 0;
        }
#endif
            ...
            for (j = 0; j < peerSuites->suiteSz; j += 2)
                if (ssl->suites->suites[i]   == peerSuites->suites[j] &&
                    ...
                    if (VerifyServerSuite(ssl, i)) {
                        int result;
                        printf("Verified suite validity");
                        ...
                        printf("Could not verify suite validity, continue");
                    }
                }
                              ...
                              word32 inSz, word16 sz)
    {
        ...
        (void)inSz;
        printf("Got old format client hello");
#ifdef WOLFSSL_CALLBACKS
        if (ssl->hsInfoOn)
        ...
        if (ssl->version.minor > pv.minor) {
            ...
            byte havePSK = 0;
            if (!ssl->options.downgrade) {
                printf("Client trying to connect with lesser version");
                return VERSION_ERROR;
            }
            if (pv.minor < ssl->options.minDowngrade) {
                printf("    version below minimum allowed, fatal error");
        ...
        *inOutIdx = idx;
        ...
        if (ssl->options.resuming) {  /* let's try */
            ...
            if (!session) {
                printf("Session lookup for resume failed");
                ssl->options.resuming = 0;
            } else {
                if (MatchSuite(ssl, &clSuites) < 0) {
                    printf("Unsupported cipher suite, OldClientHello");
                    return UNSUPPORTED_SUITE;
                }
                #ifdef SESSION_CERTS
                             ...
                             word32 helloSz)
    {
        ...
        if (b == ID_LEN) {
            ...
            printf("Client wants to resume session");
        }
        else if (b) {
            printf("Invalid session ID size");
        ...
        if (ssl->options.usingCompression) {
            ...
            if (!match) {
                printf("Not matching compression, turning off");
                ssl->options.usingCompression = 0;  /* turn off */
            }
        }
        else
        ...
        *inOutIdx = i;
        ...
        if (ssl->options.resuming) {
            ...
            if (!session) {
                ...
                if (MatchSuite(ssl, &clSuites) < 0) {
                    printf("Unsupported cipher suite, ClientHello");
                    return UNSUPPORTED_SUITE;
                }
                #ifdef SESSION_CERTS
                    ssl->session = *session; /* restore session certs. */
                                   ...
                                   word32 size)
    {
        ...
        *inOutIdx += OPAQUE16_LEN;
        ...
        if (ssl->peerRsaKey != NULL && ssl->peerRsaKeyPresent != 0) {
            ...
            printf("Doing RSA peer cert verify");
            ...
            if (IsAtLeastTLSv1_2(ssl)) {
                ...
                if (sigAlgo != rsa_sa_algo) {
                    printf("Oops, peer sent RSA key but not in verify");
                }
        ...
        if (ssl->peerEccDsaKeyPresent) {
            ...
            printf("Doing ECC peer cert verify");

            if (IsAtLeastTLSv1_2(ssl)) {
                if (sigAlgo != ecc_dsa_sa_algo) {
                    printf("Oops, peer sent ECC key but not in verify");
                }
...
#define WOLFSSL_TICKET_ENC_SZ (SESSION_TICKET_LEN - WOLFSSL_TICKET_FIXED_SZ)
    ...
    static int CreateTicket(WOLFSSL* ssl)
    {
        ...
        if (ret == WOLFSSL_TICKET_RET_OK) {
            if (encLen < (int)sizeof(InternalTicket) ||
                ...
                printf("Bad user ticket encrypt size");
            ...
            if (XMEMCMP(et->enc_ticket, &it, sizeof(InternalTicket)) == 0) {
                printf("User ticket encrypt didn't encrypt");
            ...
            if (XMEMCMP(et->key_name, zeros, WOLFSSL_TICKET_NAME_SZ) == 0) {
                printf("User ticket encrypt didn't set name");
                return BAD_TICKET_ENCRYPT;
            }

            /* iv */
            if (XMEMCMP(et->iv, zeros, WOLFSSL_TICKET_IV_SZ) == 0) {
                printf("User ticket encrypt didn't set iv");
                return BAD_TICKET_ENCRYPT;
            }

            /* mac */
            if (XMEMCMP(et->mac, zeros, WOLFSSL_TICKET_MAC_SZ) == 0) {
                printf("User ticket encrypt didn't set mac");
                return BAD_TICKET_ENCRYPT;
            }
    ...
    int DoClientTicket(WOLFSSL* ssl, const byte* input, word32 len)
    {
        ...
        if (outLen > inLen || outLen < (int)sizeof(InternalTicket)) {
            printf("Bad user ticket decrypt len");
            return BAD_TICKET_KEY_CB_SZ;
        }
                                                                    ...
                                                                    word32 size)
    {
        ...
        if (ssl->options.side != WOLFSSL_SERVER_END) {
            printf("Client received client keyexchange, attack?");
            WOLFSSL_ERROR(ssl->error = SIDE_ERROR);
            return SSL_FATAL_ERROR;
        }

        if (ssl->options.clientState < CLIENT_HELLO_COMPLETE) {
            printf("Client sending keyexchange at wrong time");
            SendAlert(ssl, alert_fatal, unexpected_message);
            return OUT_OF_ORDER_E;
        }
            ...
            if (ssl->options.verifyPeer && ssl->options.failNoCert) {
                if (!ssl->options.havePeerCert) {
                    printf("client didn't present peer cert");
                    return NO_PEER_CERT;
                }
            }
        ...
        switch (ssl->specs.kea) {
                    ...
                    if (ssl->options.tls) {
                        ...
                        *inOutIdx += OPAQUE16_LEN;
                        ...
                        if ((word32) check != length) {
                            printf("RSA explicit size doesn't match");
                            wc_FreeRsaKey(&key);
            ...
            case psk_kea:
            {
                ...
                if (ssl->options.server_psk_cb == NULL) {
                    printf("No server PSK callback set");
                    return PSK_KEY_ERROR;
                }
            ...
            case ecc_diffie_hellman_kea:
            {
                ...
                *inOutIdx += length;
                ...
                if (ssl->specs.static_ecdh) {
                    ...
                    if (ssl->eccTempKeyPresent == 0) {
                        printf("Ecc ephemeral key not made correctly");
    ...
    static int SNI_Callback(WOLFSSL* ssl)
    {
        ...
        * when SNI is received. Call it now if exists */
        if(ssl && ssl->ctx && ssl->ctx->sniRecvCb) {
            printf("Calling custom sni callback");
            if(ssl->ctx->sniRecvCb(ssl, NULL, ssl->ctx->sniRecvCbArg)
                    == alert_fatal) {
                printf("Error in custom sni callback. Fatal alert");
