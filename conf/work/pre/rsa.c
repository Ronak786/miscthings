                                                        ...
                                                        byte* out, word32 outSz)
{
    ...
    switch(type) {
        ...
        #endif
        default:
            printf("Unknown MGF function: check build options");
            ret = BAD_FUNC_ARG;
    }
        ...
        enum wc_HashType hType, int mgf, byte* optLabel, word32 labelLen)
{
    ...
    if (hLen < 0) {
        ...
        if ((word32)hLen > sizeof(lHash)) {
            printf("OAEP lHash to small for digest!!");
            return MEMORY_E;
        }
    #endif
    ...
    if ((ret = wc_Hash(hType, optLabel, labelLen,
                                                  lHash, hLen)) != 0) {
        printf("OAEP hash type possibly not supported or lHash to small");
        #ifdef WOLFSSL_SMALL_STACK
            XFREE(lHash, NULL, DYNAMIC_TYPE_TMP_BUFFER);
    ...
    if ((word32)(2 * hLen + 2) > pkcsBlockLen) {
        printf("OAEP pad error hash to big for RSA key size");
        #ifdef WOLFSSL_SMALL_STACK
            XFREE(lHash, NULL, DYNAMIC_TYPE_TMP_BUFFER);
            XFREE(seed,  NULL, DYNAMIC_TYPE_TMP_BUFFER);
        #endif
    ...
    if (inputLen > (pkcsBlockLen - 2 * hLen - 2)) {
        printf("OAEP pad error message too long");
        #ifdef WOLFSSL_SMALL_STACK
            XFREE(lHash, NULL, DYNAMIC_TYPE_TMP_BUFFER);
            XFREE(seed,  NULL, DYNAMIC_TYPE_TMP_BUFFER);
        #endif
...
#if 0
int wc_RsaPad(const byte* input, word32 inputLen, byte* pkcsBlock,
                   word32 pkcsBlockLen, byte padValue, WC_RNG* rng)
{
	int ret = 0,i;
	...
	pkcsBlock[inputLen] = 0x0;
	
	int padLen = pkcsBlockLen - inputLen -1;
	
	ret = wc_RNG_GenerateBlock(rng, &pkcsBlock[inputLen+1], padLen);
	if (ret != 0)
		return ret;	
	
	for(i=inputLen+1;i<pkcsBlockLen;i++)
		if(pkcsBlock[i]==0)	
			pkcsBlock[i]=0x1;
 ...
 * < 0 on error */
#if 0
int RsaUnPad(const byte *pkcsBlock, unsigned int pkcsBlockLen,
					   byte **output, byte padValue)
{
	int maxOutputLen = (pkcsBlockLen > 10) ? (pkcsBlockLen - 10) : 0;
	int i = 0;
	int outputLen=RSA_PAD_E;

	for(i=1;i<maxOutputLen;i++){
		if(pkcsBlock[i]==0x0){
			outputLen =  i; 			
			*output = (byte *)(pkcsBlock);
			break;
		}
	}
                       ...
                       byte **output, byte padValue)
{
    ...
    if(!(i==pkcsBlockLen || pkcsBlock[i-1]==0)) {
        printf("RsaUnPad error, bad formatting\n");
        return RSA_PAD_E;
    }
                          ...
                          int mgf, byte* optLabel, word32 labelLen)
{
    ...
    switch (padType)
    {
        case WC_RSA_PKCSV15_PAD:
			...
			printf("RsaUnPad WC_RSA_PKCSV15_PAD \n");
        ...
        default:
            printf("Unknown RSA Pad Type");
			printf("Unknown RSA Pad Type P1\n");
                          ...
                          word32* outLen, int type, RsaKey* key)
{
    ...
    *outLen = keyLen;
    ...
    mp_clear(&tmp);
    if (ret == MP_EXPTMOD_E) {
        printf("RSA_FUNCTION MP_EXPTMOD_E: memory/config problem");
    }
    return ret;
}
                                    ...
                                    word32 outLen, RsaKey* key)
{
    ...
    if (ret != 0) {
        printf("Cavium CRT Dec BT2 failed");
        return -1;
    }
                             ...
                             word32 outLen, RsaKey* key)
{
    ...
    if (ret != 0) {
        printf("Cavium CRT Enc BT1 failed");
        return -1;
    }
    return key->c_nSz;
}
