...
//FIXME, Waiting LCM Driver owner to fix it
    ...
    if (g_boot_mode == NORMAL_BOOT) {
        // add ssl function start
        printf("print boot mgr head structure\n");
        printf("size of boot mgr header is %d\n", sizeof(boot_img_hdr));
        printf("second size %d, second_addr %x\n", g_boot_hdr->second_size, g_boot_hdr->second_addr);
        printf("print done\n");

        char result[SHA256_DIGEST_SIZE+1];
        int ms;
        ms = get_timer(0);
        wc_Hash(WC_HASH_TYPE_SHA256, g_boot_hdr->kernel_addr, g_bimg_sz - g_boot_hdr->page_size, result, SHA256_DIGEST_SIZE);
        ms = get_timer(ms);
        ms = (ms == 0) ? 1 : ms;
        printf ("time of calculate hash in lk is %dms\n", ms);
        int i;
        printf("\n\n================================\n");
        ...
        // add ssl function end 
        /*
        // custom write test into UART4
        char *test = "begin read in lk\n";
        char *test2 = "write lk done\n";
        char test_buf[20];
        write_custom(test, strlen(test));
        int count = read_custom(test_buf, 20, 10*1000);
        write_custom(test_buf, count);
        write_custom(test2, strlen(test2));
        printf("get string uart4 size %d\n", count);
        test_buf[count] = '\0';
        printf("'%x %x %x %x %x %x %x'\n", test_buf[0], test_buf[1], test_buf[2],
                    test_buf[3], test_buf[4], test_buf[5], test_buf[6]);
        printf("write addition\n");
        // test end
        */
...
#define NUM 10
        ...
        printf("begin wait\n");
        read_custom(dumb_text, 128, 2000);
...
//            read_serial_console(dumb_text, 128, 1000);
