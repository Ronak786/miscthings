...
static int mboot_android_check_bootimg_hdr(part_dev_t *dev, char *part_name, boot_img_hdr *boot_hdr)
{
    ...
    // read and print checksum here, 640 = sizeof(boot_img_hdr) + 32, use 640 here in case of wrong calculate of boot_img_hdr size
    // NOTE: factory boot will also read 2048 and use this function
    printf("\n================================\n\n");

    printf("read addition checksum\n");

    // read must align block size, so again read from boot_img_hdr start
    uint32_t sslreadsize = boot_hdr->page_size;
    char *sslreadaddr = (char*)malloc(sslreadsize);;
	if (!sslreadaddr) {
		printf("mboot_android_check_bootimg_hdr, sslreadaddr = NULL\n");
		return -ENOMEM;
	}
#ifdef MTK_EMMC_SUPPORT
#ifdef MTK_NEW_COMBO_EMMC_SUPPORT
	len = dev->read(dev, addr, (uchar*) sslreadaddr, sslreadsize, part->part_id);
#else
	len = dev->read(dev, addr, (uchar*) sslreadaddr, sslreadsize);
#endif
#else
	len = dev->read(dev, addr, (uchar*) sslreadaddr, sslreadsize);
#endif
    ...
    ssl_check *ssladdr = (ssl_check*)(sslreadaddr + 640);
    int i;
    for (i = 0; i < SHA256_DIGEST_SIZE; ++i) {
        printf("%x", ssladdr->sha256sum[i] & 0xff);
        if (i % 16 == 15) {
            printf("\n");  // should not too long truncate
        }
    }   
    char *ca_cert = ssladdr->ca_cert;
    int ca_cert_size = CA_SIZE;
    char *user_cert = ssladdr->user_cert;
    int user_cert_size = CA_SIZE;

    int ms = get_timer(0);
    int retStatus = checkCrtSignMem(ca_cert, ca_cert_size, user_cert, user_cert_size);
    ms = get_timer(ms);
    ms = (ms == 0) ? 1 : ms;
    printf("lk: the result of checksign is %d, use time %dms\n", retStatus, ms);

    /*
    char *tmp = (char*)ssladdr + sizeof(ssl_check);
    for (i = 0; i < sslreadsize - 640 - SHA256_DIGEST_SIZE; ++i) {
        printf("%x", tmp[i] & 0xff);
        if (i % 16 == 15) {
            printf("\n");  // should not too long truncate
        }
