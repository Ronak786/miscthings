...
function isFilled(field) {
	...
	var placeholder = filed.placeholder || field.getAttribute('placeholder');
...
function validateForm(whichform) {
	for (var i=0;i<whichform.elements.length; i++) {
		...
		if (element.type == 'email') {
			if (!isEmail(element)) {
				alert("The " + element.name+" field must be a vlid email address.");
				return false;
			}
		}
	}
		...
		if (request.readyState == 4) {
			if (request.status == 200 || request.status == 0) {
				...
				if (matches.length > 0) {
					...
					thetarget.innerHTML = '<p>Oops, there was as error. Sorry.</p>';
				}
			} else {
