...
void Assert(const char* test, string check)
{
	...
	try
	{	
		auto node = Parse(test, &numhdl);   //here we get an expression tree
		node->Accept(&visitor);	  //here we take the tree and recursively make into an expression string properly
	}
	catch (Exception)
	{
